...
- MCP Registry server definitions are discovery and configuration metadata, not trusted proof of tools, accountable ownership, authentication, data handling, security review, oversight, or operations. The static site must not fetch packages/endpoints, resolve templates, execute MCP servers, retain secret values, or claim full official-schema validation.
