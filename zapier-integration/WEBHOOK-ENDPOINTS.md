...
> **Unsupported prototype:** the hostnames below are historical examples, not verified deployment status. Do not deploy this service as part of Samsarix Agent Readiness Registry. A local instance now refuses to start unless `HELIX_WEBHOOK_SECRET` is at least 32 characters, and every `/webhook/*` request must send `Authorization: Bearer <HELIX_WEBHOOK_SECRET>`. The variable retains its historical name for compatibility; keep that secret in the deployment platform's secret store.
