...
const lifecycle = String(
  process.env.INPUT_LIFECYCLE || "review,production",
).trim();
const requireCandidates = String(process.env.INPUT_REQUIRE_CANDIDATES || "true")
  .trim()
  .toLowerCase();
...
const annotationErrorStream = {
  write(value) {
    console.error(
      `::error title=Agent readiness action::${githubMessage(String(value).trimEnd())}`,
    );
  },
};
...
if (!registry) {
  console.error(
    "::error title=Agent readiness action::The registry input is required.",
  );
  process.exitCode = EXIT_CODES.usage;
} else if (!["true", "false"].includes(requireCandidates)) {
  console.error(
    "::error title=Agent readiness action::require-candidates must be true or false.",
  );
  process.exitCode = EXIT_CODES.usage;
} else {
  const args = [
