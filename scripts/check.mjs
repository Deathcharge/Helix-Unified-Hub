...
try {
  ...
  readyExample = normalizeRegistryDocument(JSON.parse(await read('docs/review-ready-registry-example.json')));
} catch (error) {
...
const [landingPage, registryPage, ciGuide, actionMetadata, packageText, lockText, pagesWorkflow] = await Promise.all([
...
if (!landingPage.includes('href="CI_INTEGRATION.md"')) fail('docs/index.html: CI guide is not discoverable.');
if (!registryPage.includes('href="review-ready-registry-example.json"')) fail('docs/registry.html: ready example is not discoverable.');
if (!landingPage.includes('v1.2 release candidate') || !registryPage.includes('v1.2 release candidate')) {
  fail('Primary product pages must display the v1.2 release identity.');
}
if (!ciGuide.includes('contents: read') || !ciGuide.includes('Exit contract')) fail('docs/CI_INTEGRATION.md: least-privilege workflow or exit contract is missing.');
if (!actionMetadata.includes('using: node24') || !actionMetadata.includes('main: bin/samsarix-action.mjs')) {
  fail('action.yml: Node runtime entry point is inconsistent.');
}
if (/\btoken\s*:/i.test(actionMetadata)) fail('action.yml: the readiness action must not declare a token input.');
if (!pagesWorkflow.includes('node-version: 24') || !pagesWorkflow.includes('uses: ./')) fail('Pages validation must use Node 24 and exercise the local readiness Action.');
for (const match of pagesWorkflow.matchAll(/^\s*uses:\s*([^\s#]+)/gm)) {
  if (!match[1].startsWith('./') && !/^[^@\s]+@[0-9a-f]{40}$/.test(match[1])) {
    fail(`Pages workflow action is not pinned to an immutable revision: ${match[1]}.`);
  }
}
const packageMetadata = JSON.parse(packageText);
const packageLock = JSON.parse(lockText);
if (packageMetadata.version !== '1.2.0-rc.1' || packageLock.version !== packageMetadata.version) fail('Package and lockfile release versions are inconsistent.');
if (packageMetadata.engines?.node !== '>=22' || packageLock.packages?.['']?.engines?.node !== '>=22') fail('Package and lockfile Node support floors are inconsistent.');
if (packageMetadata.bin?.['samsarix-registry'] !== 'bin/samsarix-registry.mjs') fail('package.json: registry executable is not declared.');
if (packageMetadata.scripts?.registry !== 'node bin/samsarix-registry.mjs') fail('package.json: registry script is inconsistent.');
...
if (readyExample) {
  if (readyExample.agents.length !== 1 || readyExample.agents[0].lifecycle !== 'review') fail('The ready example must contain one review candidate.');
  if (evaluateAgent(readyExample.agents[0], { now: new Date('2026-08-09T00:00:00Z') }).status !== 'ready') {
    fail('The ready example must pass at the documented reproducible evaluation date.');
  }
}
...
if (!license.includes('Version 1.2.0-rc.1')) fail('LICENSE: release version is inconsistent.');
