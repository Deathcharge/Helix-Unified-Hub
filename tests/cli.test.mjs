...
test('argument parser defines conservative deployment candidates and reproducible time', () => {
  const parsed = parseCliArguments([
    'check', 'registry.json', '--include-development', '--require-candidates', '--now', '2026-08-09'
  ]);
...
test('validate emits stable inventory metadata for a Samsarix registry', async () => {
  const result = await invoke(['validate', 'docs/agents.json', '--format', 'json']);
...
test('stdin accepts bounded A2A metadata without network access', async () => {
  const card = await readFile(path.join(root, 'docs', 'a2a-agent-card-example.json'), 'utf8');
  const result = await invoke(['validate', '-', '--format', 'json'], { stdin: Readable.from([card]) });
  assert.equal(result.code, EXIT_CODES.success);
...
test('check passes a complete review candidate and fails a blocked development candidate', async () => {
  const passing = await invoke([
    'check', 'docs/review-ready-registry-example.json', '--now', '2026-08-09', '--format', 'json'
  ]);
  assert.equal(passing.code, EXIT_CODES.success);
  assert.equal(JSON.parse(passing.stdout).agents[0].score, 100);

  const blocked = await invoke([
    'check', 'docs/a2a-agent-card-example.json', '--lifecycle', 'development', '--now', '2026-08-09', '--format', 'json'
  ]);
  assert.equal(blocked.code, EXIT_CODES.readiness);
  const result = JSON.parse(blocked.stdout);
  assert.equal(result.passed, false);
  assert.equal(result.summary.failed, 1);
  assert.ok(result.agents[0].blockers.some((entry) => entry.includes('Risk tier is unassessed')));
});

test('check ignores concepts by default and can require a deployment candidate', async () => {
  const inventory = await invoke(['check', 'docs/agents.json', '--now', '2026-08-09']);
  assert.equal(inventory.code, EXIT_CODES.success);
  assert.match(inventory.stdout, /Candidates: 0\/0 ready/);

  const required = await invoke(['check', 'docs/agents.json', '--require-candidates', '--now', '2026-08-09']);
...
test('GitHub format escapes annotation properties and reports blockers', async () => {
  const source = JSON.parse(await readFile(path.join(root, 'docs', 'a2a-agent-card-example.json'), 'utf8'));
  ...
  const result = await invoke([
    'check', fixture, '--lifecycle', 'development', '--format', 'github', '--now', '2026-08-09'
  ]);
...
test('report output is deterministic for normalized JSON and Markdown', async () => {
  const jsonFirst = await invoke(['report', 'docs/review-ready-registry-example.json', '--format', 'json']);
  const jsonSecond = await invoke(['report', 'docs/review-ready-registry-example.json', '--format', 'json']);
  assert.equal(jsonFirst.code, EXIT_CODES.success);
  assert.equal(jsonFirst.stdout, jsonSecond.stdout);
  const markdown = await invoke([
    'report', 'docs/review-ready-registry-example.json', '--format', 'markdown', '--now', '2026-08-09'
  ]);
...
test('usage, invalid data, oversized input, and missing input have distinct exits', async () => {
  ...
  assert.equal((await invoke(['validate', path.join(temporary, 'missing.json')])).code, EXIT_CODES.input);
});
...
test('executable entry point provides help, version, and exact readiness exit codes', () => {
  ...
  const version = spawnSync(process.execPath, [cli, '--version'], { cwd: root, encoding: 'utf8' });
  ...
  const pass = spawnSync(process.execPath, [
    cli, 'check', 'docs/review-ready-registry-example.json', '--now', '2026-08-09'
  ], { cwd: root, encoding: 'utf8' });
  assert.equal(pass.status, EXIT_CODES.success);

  const fail = spawnSync(process.execPath, [
    cli, 'check', 'docs/agents.json', '--require-candidates', '--now', '2026-08-09'
  ], { cwd: root, encoding: 'utf8' });
